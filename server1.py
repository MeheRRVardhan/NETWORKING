...
def generate_traffic(server_url, health_list, memory_list, packet_size):
    ...
    while True:
        ...
        try:
            for _ in range(packet_size):  # Send user-specified number of requests
        ...
        # Save health report for display in server1.html
...
@app.route('/')
def index():
    return render_template('server1.html', health_reports=health_reports)
